...
zip -q -r $BASE_PATH/PatientMonitorDataLogger.API.zip publish

cd $BASE_PATH/patient-monitor-datalogger-frontend
npm run build
zip -q -r $BASE_PATH/PatientMonitorDataLogger.Frontend.zip dist
cd $BASE_PATH

#scp ~/git/patient-monitor-datalogger/PatientMonitorDataLogger.*.zip datalogger-3bplus:~
#ssh datalogger-3bplus "./deploy_patient-monitor-datalogger.sh"

scp ~/git/patient-monitor-datalogger/PatientMonitorDataLogger.*.zip datalogger-4b:~
